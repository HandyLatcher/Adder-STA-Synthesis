`timescale 1ns/1ps

module ripple_adder_tb;
    // 1. Parameters
    parameter WIDTH = 8;
    parameter RANDOM_TESTS = 50;

    // 2. Signals
    reg signed [WIDTH-1:0] A, B;
    reg Cin;
    wire signed [WIDTH-1:0] Sum;
    wire Cout;

    integer pass_count = 0;
    integer fail_count = 0;

    // 3. Golden Model
    wire signed [WIDTH:0] expected_out = A + B + Cin;

    // 4. DUT
    ripple_adder #(.WIDTH(WIDTH)) uut (
        .A(A), .B(B), .Cin(Cin), 
        .Sum(Sum), .Cout(Cout)
    );

    // 5. Tasks
    task check_adder(input signed [WIDTH-1:0] t_a, input signed [WIDTH-1:0] t_b, input t_cin); //direct use of ABCin might lead to corruption suppose if use ABCin in header then if do
        begin				    //A=A+1 inside task it will change actual value also
            A = t_a; B = t_b; Cin = t_cin;
            #10; //its used because dut only updates once this task finishes
            if ({Cout, Sum} !== expected_out) begin
                $display("FAIL @ %0t | A=%d, B=%d, Cin=%b | Expected=%d, Got=%d", 
                         $time, A, B, Cin, expected_out, {Cout, Sum});
                fail_count++;
           end else begin
                pass_count++;
            end
        end
    endtask

    // 6. Execution
    initial begin
        $dumpfile("rca.vcd"); $dumpvars(0,ripple_adder_tb);

        // Edge Cases
        check_adder(8'sd127, 8'sd1, 0);
        check_adder(8'h80, 8'hFF, 0);
        check_adder(8'hAA, 8'h55, 1);
        
        // Random Stress Tests
        repeat(RANDOM_TESTS) begin
            check_adder($random, $random, $random % 2);
        end

        // Simulation Summary
        $display("\n==============================");
        $display("      Simulation Summary      ");
        $display("==============================");
        $display("Total Tests : %0d", pass_count + fail_count);
        $display("Passed      : %0d", pass_count);
        $display("Failed      : %0d", fail_count);
        $display("==============================\n");
        $finish;
    end
endmodule
